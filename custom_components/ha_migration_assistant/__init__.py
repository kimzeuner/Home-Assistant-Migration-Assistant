...
from .const import (
    ...
    SERVICE_EXPORT_PLAN,
    SERVICE_RESCAN,
    SERVICE_GENERATE_DIFF,
    SIGNAL_SCAN_UPDATED,
)
from .scanners import MigrationScanResult, scan_config
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if not hass.services.has_service(DOMAIN, SERVICE_GENERATE_DIFF):
        hass.services.async_register(
            DOMAIN,
            SERVICE_GENERATE_DIFF,
            _async_handle_generate_diff_service,
            schema=vol.Schema(
                {
                    vol.Optional(ATTR_ENTRY_ID): str,
                    vol.Optional(ATTR_FILENAME): str,
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if unload_ok:
        ...
        if not hass.data.get(DOMAIN):
            ...
            hass.services.async_remove(DOMAIN, SERVICE_GENERATE_DIFF)
    return unload_ok
...
async def _async_handle_generate_diff_service(call: ServiceCall) -> None:
    hass = call.hass
    entry_id = call.data.get(ATTR_ENTRY_ID)
    filename = call.data.get(ATTR_FILENAME)
    entries = _get_target_entries(hass, entry_id)
    if not entries:
        _LOGGER.warning("Diff requested for unknown entry_id %s", entry_id)
        return
    ...
    for entry in entries:
        ...
        if result is None:
            _LOGGER.warning("No migration scan result available for %s", entry.title)
            continue
        diff_path = await hass.async_add_executor_job(
            _write_migration_diff,
            hass,
            result,
            filename,
        )
        hass.data[DOMAIN][entry.entry_id].last_diff_path = diff_path
        async_dispatcher_send(hass, f"{SIGNAL_SCAN_UPDATED}_{entry.entry_id}")
        _LOGGER.info("Migration diff exported for %s to %s", entry.title, diff_path)
...
def _write_migration_diff(
    ...
    export_path = export_dir / f"{safe_name}.diff"
    lines: list[str] = []
    for match in result.matches:
        lines.extend(
            [
                f"--- {match.path}",
                f"+++ {match.path}",
                f"@@ line {match.line}, column {match.column} | {match.category} | confidence {match.confidence}% @@",
                f"- {match.current}",
                f"+ {match.replacement}",
                "",
            ]
    ...
    if not lines:
        lines = [
            f"# No references to {result.old_entity_id} were found.",
            f"# Proposed replacement: {result.old_entity_id} -> {result.new_entity_id}",
        ]
